	...
	<div class="card-block">
		<div class="custom-callout custom-callout-midnight-blue gem-h5">
			<strong>
				...
				<p>{!! trans_choice('messages.program', 1).': ' !!}<span class="text-default">{!! $item->program_id !!}</span></p>
				<p>{!! trans('messages.material').': ' !!}<span class="text-primary">{!! $item->material_id !!}</span></p>
				<p>{!! trans_choice('messages.pt-round', 1).': ' !!}<span class="text-primary">{!! $item->round_id !!}</span></p>
        <p>{!! trans('messages.prepared-by').': ' !!}<span class="text-primary">{!! $item->prepared_by !!}</span></p>
			</strong>
		</div>
	</div>
</div>
