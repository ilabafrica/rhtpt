...
<div class="card">
	<div class="card-header">
  	...
  	<div class="card-block">
        ...
        @endif
		<div class="row">
			{!! Form::model($field, array('route' => array('field.update', $field->id), 'method' => 'PUT', 'id' => 'form-edit-field', 'class' => 'form-horizontal')) !!}
			<!-- CSRF Token -->
        ...
        <div class="options"{!! count($field->options)==0?'style="display:none;"':'' !!}>
          <div class="form-group row">
              ...
              <div class="col-sm-6">
                <div class="card card-block">
                  ...
                  @foreach($options as $k => $val)
                        ...
                        {{--*/ $cnt++ /*--}}
                        <div class="col-md-3">
                          <label class="checkbox-inline">{!! Form::checkbox('opt[]', $k, in_array($k, $field->options->lists('id')->toArray())?'checked':'') !!}{{ $val }}</label>
                        </div>
                        {!! ($cnt%4==0)?"</div>":"" !!}
                  @endforeach
                </div>
        ...
        <div class="form-group row">
            <div class="col-sm-offset-4 col-sm-6">
                <label class="checkbox-inline">
                    {!! Form::checkbox("is_matrix", '1', (!empty($field->is_matrix))?'checked':'', array('onclick' => 'untoggle(".matrix", this)')) !!}{{ trans('messages.matrix') }}
                </label>
            </div>
        </div>
        <div class="matrix"{!! (empty($field->is_matrix))?'style="display:none;"':'' !!}>
          <div class="form-group row">
