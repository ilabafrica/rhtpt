                ...
                <div class="modal-body">
                    <form method="POST" enctype="multipart/form-data" v-on:submit.prevent="approveUser(someUser.id)">
                        <div class="row">
                            ...
                            <div class="form-group row col-sm-offset-4 col-sm-8">
                                <button type="submit" class="btn btn-sm btn-success" v-if="someUser.status = 1"><i class='fa fa-plus-circle'></i> Enroll</button>
                                <button type="button" class="btn btn-sm btn-danger" v-if="someUser.status = 0" @click="denyUser(someUser.id)"><i class='fa fa-ban'></i> Reject</button>
                                <button type="button" class="btn btn-sm btn-silver" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><i class="fa fa-times-circle"></i> {!! trans('messages.cancel') !!}</span></button>
                            </div>
                        </div>
                    </form>
